...
@patch('math.sqrt')
def test_execute_very_small_positive_number(mock_sqrt):
    mock_sqrt.return_value = 1e-10
    assert SquareRoot.execute(1e-20) == 1e-10

@patch('math.sqrt')
def test_execute_very_large_positive_number(mock_sqrt):
    mock_sqrt.return_value = 1e10
    assert SquareRoot.execute(1e20) == 1e10